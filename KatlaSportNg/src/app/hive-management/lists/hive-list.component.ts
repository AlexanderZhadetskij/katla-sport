...
export class HiveListComponent implements OnInit {
  ...
  onDelete(hiveId: number) {
    ...
    this.hiveService.setHiveStatus(hiveId, true).subscribe(_ => hive.isDeleted = true);
  }

  onRestore(hiveId: number) {
    var hive = this.hives.find(h => h.id == hiveId);
    this.hiveService.setHiveStatus(hiveId, false).subscribe(_ => hive.isDeleted = false);
  }
}
